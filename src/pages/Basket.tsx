  ...
  return (
    <div className="p-16">
      ...
      <div className="space-y-5">
        <h2 className="text-[30px] leading-[60px] font-bold mb-5">Accessories</h2>
        <ul className="flex items-end gap-[41px] flex-wrap mb-5">
          {savedProducts.map((item: AccessoryType) => (
            <li key={item.id} className="w-[295px] text-center">
              ...
              <button onClick={() => dispatch(deleteOrderProducts(item.id))} className="w-[129px] py-[13px] inline-block border-[1px] border-[#C6C6C6] text-[13px] leading-[15px] text-[#C6C6C6] hover:border-redbg-red-500 hover:bg-red-500 hover:text-white duration-200">REMOVE</button>
            </li>
          ))}
        </ul>
        <h2 className="text-[30px] leading-[60px] font-bold mb-5">Scooters</h2>
        <ul className="flex items-center gap-[41px] flex-wrap">
          {savedScooters.map((item: ScooterType) => (
            <li key={item.id} className="w-[295px] text-center">
              ...
              <button onClick={() => dispatch(deleteOrderScooters(item.id))} className="w-[129px] py-[13px] inline-block border-[1px] border-[#C6C6C6] text-[13px] leading-[15px] text-[#C6C6C6] hover:border-redbg-red-500 hover:bg-red-500 hover:text-white duration-200">REMOVE</button>
            </li>
          ))}
        </ul>
      </div>
